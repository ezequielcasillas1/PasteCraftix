...
serve(async (req) => {
  ...
  try {
    ...
    const categorizeWorkflow: { provider: AiWorkflowProvider; preset: AiWorkflowPreset } =
      parseAiWorkflowFromBody(body) || { provider: 'openai', preset: 'cheapest' }
    const models = resolveModelsFromWorkflow(categorizeWorkflow)
    ...
    if (mode === 'suggestions') {
      ...
      const credits = await decrementTextCredits(
        gate,
        getTextCreditCost(categorizeWorkflow.provider, categorizeWorkflow.preset),
      )
