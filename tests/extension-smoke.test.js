...
const optionalWebAccessibleResources = new Set(["config.js", "icon.png", "assets/eye.gif"]);
const optionalWebAccessiblePrefixes = ["lib/"];
...
function isOptionalWebAccessibleResource(relativePath) {
  return (
    optionalWebAccessibleResources.has(relativePath) ||
    optionalWebAccessiblePrefixes.some((prefix) => relativePath.startsWith(prefix))
  );
}
...
for (const group of manifest.web_accessible_resources || []) {
  for (const resource of group.resources || []) {
    ...
    if (isOptionalWebAccessibleResource(resource)) continue;
