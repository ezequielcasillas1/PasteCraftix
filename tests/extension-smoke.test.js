...
const generatedResources = new Set(["config.js", "icon.png", "assets/eye.gif"]);
...
function isGeneratedResource(relativePath) {
  return generatedResources.has(relativePath) || relativePath.startsWith("lib/");
}
...
for (const group of manifest.web_accessible_resources || []) {
  for (const resource of group.resources || []) {
    ...
    if (isGeneratedResource(resource)) continue;
