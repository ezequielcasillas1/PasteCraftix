...
**`qp.render.js`** — extract `createInterface` / `renderClips` / `showInterface` / `updateInterface` helpers from `content/quick-paste/quick-paste.js`. Low–medium risk; same-area chain after `qp.storage.js`.
