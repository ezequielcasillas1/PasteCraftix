...
**`qp.render.js`** — extract `createInterface`, `renderClips`, `showInterface`, `hideInterface`, `updateInterface`, `applySettings` from `content/quick-paste/quick-paste.js`. Low–medium risk; same-area chain after `qp.storage.js`.
