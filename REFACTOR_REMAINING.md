...
**`qp.clips-actions.js`** — extract `showClearAllConfirmation`, `clearAllClips`, `toggleClipSelection`, `copyMultipleClips`, `deleteClip` / `deleteClipById` from `content/quick-paste/quick-paste.js`. Medium risk; same-area chain after `qp.settings-modal.js`.
...
1. **quick-paste** `quick-paste.js` → `qp.*` submodules
   - Done: `qp.helpers.js`, `qp.constants.js`, `qp.styles.js`, `qp.storage.js`, `qp.render.js`, `qp.events.js`, `qp.paste.js`, `qp.settings-modal.js`
   - Next: `qp.clips-actions.js` ← **start here**
   - Then: `qp.controller.js` shell
