...
**`qp.paste.js`** — extract `pasteClip`, `pasteClipById`, `showToast` from `content/quick-paste/quick-paste.js`. Medium risk; same-area chain after `qp.events.js`.
...
1. **quick-paste** `quick-paste.js` → `qp.*` submodules
   - Done: `qp.helpers.js`, `qp.constants.js`, `qp.styles.js`, `qp.storage.js`, `qp.render.js`, `qp.events.js`
   - Next: `qp.paste.js` ← **start here**
   - Then: `qp.settings-modal.js`, `qp.clips-actions.js`, `qp.controller.js` shell
