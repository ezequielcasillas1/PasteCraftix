...
**`qp.settings-modal.js`** — extract `showSettingsModal`, settings modal events/save, help modal from `content/quick-paste/quick-paste.js`. Medium–High risk; same-area chain after `qp.paste.js`.
...
1. **quick-paste** `quick-paste.js` → `qp.*` submodules
   - Done: `qp.helpers.js`, `qp.constants.js`, `qp.styles.js`, `qp.storage.js`, `qp.render.js`, `qp.events.js`, `qp.paste.js`
   - Next: `qp.settings-modal.js` ← **start here**
   - Then: `qp.clips-actions.js`, `qp.controller.js` shell
