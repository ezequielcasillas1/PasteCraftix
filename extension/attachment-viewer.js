...
function isAllowedImageSrc(src) {
  ...
  if (typeof PCSanitize !== 'undefined' && typeof PCSanitize.isAllowedImageSrc === 'function') {

    return PCSanitize.isAllowedImageSrc(src);
...
function buildUrlDisplayHtml(url) {

  if (typeof PCSanitize !== 'undefined' && typeof PCSanitize.buildSafeUrlDisplayHtml === 'function') {

    return PCSanitize.buildSafeUrlDisplayHtml(url, escapeHtml);
  ...
  if (!/^https?:\/\//i.test(raw)) {
    ...
    return `<span class="pc-url-display">${escapeHtml(raw)}</span>`;
...
async function main() {
  ...
  if (att.type === 'image') {
    ...
    if (!src || !isAllowedImageSrc(src)) {
      ...
      if (bodyEl) bodyEl.innerHTML = '<pre>Image attachment is missing a source or uses a blocked URL scheme.</pre>';
