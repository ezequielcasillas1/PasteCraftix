...
function resolveTheme(theme, currentTheme) {
  if (theme === QP_DEFAULTS.THEME_DARK || theme === QP_DEFAULTS.THEME_LIGHT) {
    return theme;
  }
  if (currentTheme !== QP_DEFAULTS.THEME_DARK) {
...
/**
 * Load settings + position; theme follows global storage key.
...
export async function loadQuickPasteSettings(currentSettings, currentPosition) {
  ...
  try {
    ...
    settings.theme = resolveTheme(result[QP_STORAGE_KEYS.THEME], settings.theme);
